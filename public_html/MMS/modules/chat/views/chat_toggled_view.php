      ...
      <div class="chat-footer" style="background:<?php echo $currentChatColor; ?>">
        ...
        <div class="form-inline colorHolder">
          <form method="POST" name="solidForm" style="display: none"
                action="<?php echo site_url('chat/chat_Controller/colorchange/'); ?>"
    ...
    <div class="pusherChatBox">
      <span class="state">
        <span class="userIsTyping"><img
            src="<?php echo module_dir_url('chat', 'assets/chat_implements/userIsTyping.gif'); ?>"/></span>
        <span class="quote">
...
<script>
  ...
  function uploadFileForm(file) {
    ...
    $.ajax({
      type: "POST",
      url: '<?php echo site_url('chat/chat_Controller/uploadMethod'); ?>',
      data: formData,
      dataType: "json",
      processData: false,
      contentType: false,
      ...
      success: function (r) {
        ...
        var basePath = "<?php echo module_dir_url('chat', 'uploads/'); ?>";
  ...
  var pusher = new Pusher(pusherKey, {
    authEndpoint: "<?php echo site_url('chat/chat_Controller/pusher_auth'); ?>",
    authTransport: "jsonp",
    "cluster": appCluster,
    disableStats: true,
    auth: {
  ...
  presenceChannel.bind("send-event", function (data) {
    ...
    console.log("Current user ID:", presenceChannel.members.me.id);
    console.log("Intended recipient ID:", data.to);
    if (data.global) {
    ...
    // Perform the replace operation and assign to a new variable
var modifiedFrom = data.from.replace('client_', '');

// Now log the modified value
console.log("Modified from (without 'client_'):", modifiedFrom);
    
    if (presenceChannel.members.me.id == modifiedFrom) {
    data.message = createTextLinks_(emojify.replace(data.message));
    $("#pusherChat .pusherChatBox#id_" + data.to.replace('staff_', '') + " .msgTxt").append('<div class="conversation_me"><img class="myProfilePic" data-toggle="tooltip" title="' + current_time + '" src="' + fetchUserAvatar(userSessionId, data.sender_image) + '"/><br><div class="message_container"><p class="you" data-toggle="tooltip" data-placement="top" id="' + data.last_insert_id + '">' + data.message + '</p>' + optionsMore + '</div><i class="fa-solid fa-circle-check circle-unseen" data-toggle="tooltip" data-placement="left" title="<?= _l("chat_msg_delivered"); ?>" aria-hidden="true"></i><div class="fixinline"></div></div>');

    // Check if the element exists before attempting to access its scrollHeight property
    var pusherDatalogMsgTo = $("#pusherChat .pusherChatBox#id_" + data.to.replace('staff_', '') + " .logMsg");
    if (pusherDatalogMsgTo.length > 0) {
  ...
  $("#pusherChat svg.main_chat").on("click", function () {
    window.location.href = admin_url("/chat/chat_Controller/chat_full_view");
    return false; // Prevents the default action of the event
});
