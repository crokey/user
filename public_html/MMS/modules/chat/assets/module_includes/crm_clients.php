...
<script>
  (function () {
    ...
    clientsChannel.bind("reply-client-event", function (data) {
    console.log("Received reply-client-event:", data);

    // Skip appending the message if it's from the current user
    if (data.from == "client_" + userSessionId) {
        return;
    }

    var senderImage = fetchUserAvatar(data.from.replace('client_', ''), data.sender_image);

    var originalMessageHTML = `<br><span class='original-message' onclick='scrollToMessage(${data.original_message_id})' style='color: gray;'><i class='fa-solid fa-reply' aria-hidden='true'></i><i class='fa-solid fa-reply' aria-hidden='true'></i> ${data.original_message}</span><br><br>`;
    var optionsMore = deleteOrForward(data.last_insert_id);

    var messageHTML = `<li class='${data.from == userSessionId ? 'sent' : 'replies'}' id='${data.last_insert_id}'>
                        ${data.from == userSessionId ? '<i class="fa-solid fa-circle-check circle-unseen" data-toggle="tooltip" data-container="body" data-placement="left" title="Delivered" aria-hidden="true"></i>' : ''}
                        <img data-toggle="tooltip" data-container="body" data-placement="${data.from == userSessionId ? 'top' : 'right'}" title="${moment(data.sent_time).format("YYYY-MM-DD HH:mm:ss")}" class='${data.from == userSessionId ? 'myProfilePic' : 'friendProfilePic'}' src='${senderImage}' />
                        <p class='${data.from == userSessionId ? 'you' : 'friend'}' id="${data.last_insert_id}">${originalMessageHTML + data.reply_message}</p>${optionsMore}</li>`;

    console.log("Appending message HTML:", messageHTML);
    ...
    // Hide options by default (matching the reload scenario)
    $(".chooseOption").hide();
    $(".optionsMore").hide();
});
    ...
    /*---------------* Bind the 'edit-event' & update the chat box message log *---------------*/
clientsChannel.bind("edit-event", function (data) {


    // Construct the original message HTML if it's a reply
    var originalMessageHTML = data.original_message ? `<br><span class='original-message' onclick='scrollToClientMessage(${data.original_message_id})' style='color: gray;'><i class='fa-solid fa-reply' aria-hidden='true'></i><i class='fa-solid fa-reply' aria-hidden='true'></i> ${data.original_message}</span><br><br>` : "";
    
    // Construct the edited message HTML
    var editedMessageHTML = originalMessageHTML + data.message;
    if (data.edited_time) {
        editedMessageHTML += ``;
    }

    // Find the message element by its ID
    var messageElement = $("body").find(".messages li#" + data.last_insert_id).find("p.you");
    if (messageElement.length === 0) {
        messageElement = $("body").find(".messages li#" + data.last_insert_id).find("p.client");
    }
    if (messageElement.length === 0) {
        messageElement = $("body").find(".messages li#" + data.last_insert_id).find("p.friend");
    }

    // Update the message element's HTML with the edited message
    if (messageElement.length > 0) {
        messageElement.html(editedMessageHTML);
    } else {
        console.error("Message element not found for ID:", data.last_insert_id);
    }
});
      ...
      if (value.length >= 3) {
        $("#frame  #crm_clients ul.company_selector li").each(function () {
          ...
          if (crmClientsParent.find("ul:visible:last").length === 0
            || crmClientsParent.find("ul:visible:first").length === 0) searchInDatabase = true;
        });


        if (searchInDatabase) {

          var data = {"search": $("#search_clients_field").val(),};

          $.ajax({
            type: "POST",
            url: "<?php echo site_url('chat/chat_ClientsController/searchClients'); ?>",
            cache: false,
            data: data,
    ...
    $("#frame").on("keypress", "textarea.client_chatbox", function (e) {
      ...
      if (e.which == 13 && !e.shiftKey) {
        ...
        $(".client_messages .chat_client_messages ul").append("<li class=\"sent\" id=\"" + userSessionId + '"><i class="fa-solid fa-circle-check circle-unseen" data-toggle="tooltip" data-container="body" data-placement="top" title="<?= _l('chat_msg_delivered'); ?>" aria-hidden="true"></i><img data-toggle="tooltip" title="<?= _l('chat_sent_at') . " " . _dt(date("Y-m-d H:i:s")); ?>" class="myProfilePic" src="' + imgPath + "\"/><p class=\"you\" id=\"" + userSessionId + "\">" + message + "</p>" + optionsMore + "</li>");
  ...
  function fetchMoreClients(el) {
    ...
    $.getJSON("<?php echo site_url('chat/chat_ClientsController/loadMoreClients'); ?>", {
...
if (get_option('chat_allow_staff_to_create_tickets') == 1 || is_admin() && is_staff_member()) {
  ...
  require('modules/chat/assets/module_includes/modal_additional_files_includes/convert_to_ticket_inc.php');
} ?>
