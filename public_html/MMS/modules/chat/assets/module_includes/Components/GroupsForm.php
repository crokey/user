...
<form hidden method="post" enctype="multipart/form-data" name="groupMessagesForm" id="groupMessagesForm" onsubmit="return false;">
     <div class="message-input group_msg_input">
          <div class="wrap">
               <textarea id="group_chatbox" type="text" name="g_message" class="group_chatbox ays-ignore mention" placeholder="<?= _l('chat_type_a_message_mention'); ?>"></textarea>
               ...
               <i class="fa-regular fa-file-image attachment groupFileUpload" data-container="body" data-toggle="tooltip" title="<?php echo _l('chat_file_upload'); ?>" aria-hidden="true"></i>
               <?php loadChatComponent('MicrophoneIcon'); ?>
               <button id="sendMessageBtn" class="submit enterGroupBtn" name="enterGroupBtn"><svg class="fa-paper-plane" fill="#ffffff" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function () {
    var textarea = document.getElementById('group_chatbox');
    var sendButton = document.getElementById('sendMessageBtn'); // Ensure you have this ID on your send button
  var messages = document.querySelector('.group_messages'); // Define messages here to ensure it's accessible
  ...
  // Handle sending a message when the enter key is pressed without the shift key
    textarea.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && !e.shiftKey) {
          ...
          var messages = document.querySelector('.group_messages');
