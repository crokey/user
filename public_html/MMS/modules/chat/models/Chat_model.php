...
class chat_model extends App_Model
{
    ...
    public function getUsers()
    {
      ...
      // Determine the logged-in user's full_chat_access status
    $loggedInUserId = get_staff_user_id();
    $this->db->select('full_chat_access');
    $this->db->from(db_prefix() . 'contacts'); // Assuming 'contacts' stores 'full_chat_access'
    $this->db->where('id', $loggedInUserId);
    $loggedInUserAccess = $this->db->get()->row()->full_chat_access;
      ...
      if ($loggedInUserAccess == 0) {
    // If user does not have full chat access, show only Merchant Support with a specific first name
    $this->db->where('role', '3');
    $this->db->where('firstname', 'Merchant');
      ...
      $users = $this->db->get(db_prefix() . 'staff')->result_array();
        foreach ($users as $key => $user) {
            ...
            $staffUserId = 'client_' . get_staff_user_id();
$userStaffId = 'staff_' . $user['staffid']; // Prefix as needed, change to 'client_' if it's a client ID

$sql = 'SELECT message, sender_id, time_sent FROM ' . db_prefix() . "chatclientmessages 
        WHERE (sender_id = '" . $staffUserId . "' AND reciever_id = '" . $userStaffId . "') 
           OR (sender_id = '" . $userStaffId . "' AND reciever_id = '" . $staffUserId . "') 
...
public function get_message_by_id($message_id) {
        ...
        $query = $this->db->get('chatclientmessages');
        return $query->row();
    }
  
  public function save_reply_message($originalMessageId, $replyMessage, $from, $receiver) {
    $data = [
    ...
    $this->db->insert('chatclientmessages', $data);
    return $this->db->insert_id(); // Assuming using CodeIgniter's Database Class
}
...
public function getUserIdFromContactId($contactId) {
    $this->db->select('userid');
    $this->db->from('tblcontacts');
    $this->db->where('id', $contactId); // Using 'id' to fetch the correct contact
    $query = $this->db->get();

    if ($query->num_rows() > 0) {
        return $query->row()->userid; // Return the 'userid' associated with this contact
    }
...
public function getCompanyNameByUserId($userid) {
    ...
    $this->db->where('userid', $userid); // Assuming 'userid' is the link between contacts and clients
    $query = $this->db->get();

    if ($query->num_rows() > 0) {
        return $query->row()->company; // Return the company name associated with this userid
    }
...
{
    // Keep the SQL query with placeholders for safe parameter binding
    $sql = 'SELECT * FROM ' . db_prefix() . 'chatclientmessages 
            WHERE (sender_id = ? AND reciever_id = ?) 
               OR (sender_id = ? AND reciever_id = ?) 
            ORDER BY id DESC LIMIT ?, ?';

    // Cast $limit and $offset to integers to prevent SQL injection
    $limit = (int)$limit;
    $offset = (int)$offset;

    // Execute the query with parameter bindings, including prefixes
    $query = $this->db->query($sql, [
        'client_' . $from, 'staff_' . $to,
        'staff_' . $to, 'client_' . $from,
        $offset, $limit
    ])->result();

    foreach ($query as &$chat) {
            
            $chat->sender_id = preg_replace('/^(client_|staff_)/', '', $chat->sender_id);
            $chat->reciever_id = preg_replace('/^(client_|staff_)/', '', $chat->reciever_id);
      ...
      // Fetch original message if it exists
        if ($chat->original_message_id) {
            $originalMessage = $this->db->select('message')->where('id', $chat->original_message_id)->get('chatclientmessages')->row();
            $chat->original_message = $originalMessage ? $originalMessage->message : null;
        } else {
            $chat->original_message = null;
        }
...
log_message('debug', 'Running SQL: ' . $sql);

// To log the full query with values, you can use:
log_message('debug', 'Full Query: ' . $this->db->last_query());
    ...
    public function getClientUnreadMessages()
    {
        ...
        $staff_id = 'client_' . get_staff_user_id();
        ...
        if ($query) {
            ...
            foreach ($result as $sender) {
                ...
                $contact_id = str_replace('staff_', '', $sender['sender_id']);
    ...
    public function updateUnread($pusher, $id)
{
    ...
    // Corrected SQL query with proper concatenation
    $this->db->select('id as msg_id, sender_id, reciever_id, viewed_at');
    $this->db->from(db_prefix() . "chatclientmessages");
    $this->db->where("(viewed = 0 AND reciever_id = 'client_" . $staff_id . "' AND sender_id = 'staff_" . $id . "') OR (reciever_id = 'staff_" . $id . "' AND sender_id = 'client_" . $staff_id . "' AND viewed = 0)");
    $messages = $this->db->get()->result();

    if ($messages) {
        foreach ($messages as $message) {
            // Corrected SQL query with proper concatenation
            $sql = "UPDATE " . db_prefix() . "chatclientmessages SET viewed = 1, viewed_at = '" . date('Y-m-d H:i:s') . "' WHERE ((reciever_id = 'client_" . $staff_id . "' AND sender_id = 'staff_" . $id . "') OR (reciever_id = 'staff_" . $id . "' AND sender_id = 'client_" . $staff_id . "')) AND id = '" . $message->msg_id . "'";
            $query = $this->db->query($sql);
        }
    ...
    public function get_shared_files_and_create_template($own_id, $contact_id)
    {
        ...
        foreach ($files as $file) {
            if (preg_match("/^[^\?]+\.('" . $photoExtensions . "')$/", $file)) {
                $html .= "<a data-lity href='" . base_url('modules/chat/uploads/' . $file) . "'>
               <div class='col-xs-3 shared_files_ahref' style='background-image:url(" . base_url('modules/chat/uploads/' . $file) . ");'></div></a>";
            }
        }
        $html .= '</div>';
        ...
        foreach ($files as $file) {
            ...
            if (preg_match("/^[^\?]+\.('" . $docFiles . "')$/", $file)) {
                $html .= "<div class='col-md-12'><a target='_blank' " . $data_lity . " href ='" . base_url('modules/chat/uploads/' . $file) . "'><i class='fa fa-file-o icon_shared_files' aria-hidden='true'></i>" . $file . '</a></div>';
            }
        }
        $html .= '</div></div>';
    ...
    public function get_group_shared_files_and_create_template($group_id)
    {
        ...
        foreach ($files as $file) {
            if (preg_match("/^[^\?]+\.('" . $photoExtensions . "')$/", $file)) {
                $html .= "<a data-lity href='" . base_url('modules/chat/uploads/groups/' . $file) . "'>
               <div class='col-md-3 shared_files_ahref' style='background-image:url(" . base_url('modules/chat/uploads/groups/' . $file) . ");'></div></a>";
            }
        }
        $html .= '</div>';
        ...
        foreach ($files as $file) {
            ...
            if (preg_match("/^[^\?]+\.('" . $docFiles . "')$/", $file)) {
                $html .= "<div class='col-md-12'><a target='_blank' " . $data_lity . " href ='" . base_url('modules/chat/uploads/groups/' . $file) . "'><i class='fa fa-file-o icon_shared_files' aria-hidden='true'></i>" . $file . '</a></div>';
            }
        }
        $html .= '</div></div>';
    ...
    public function handleMentionEvent($data, $pusher)
    {
        ...
        foreach ($data['users'] as $user) {
            ...
            add_notification([
                ...
                'fromcompany' => true,
                'link'        => 'chat/chat_Controller/chat_full_view',
            ]);
        }
