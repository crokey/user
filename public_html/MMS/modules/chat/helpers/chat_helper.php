...
{
  echo '<link href="' . base_url('modules/chat/assets/clients/styles.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" >';
  echo '<link href="' . base_url('modules/chat/assets/css/lity.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
}
...
function pr_chat_load_js()
{
  if ( ! strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
    echo '<script src="' . module_dir_url('chat', 'assets/js/pr-chat.js' . '?v=' . VERSIONING . '') . '"></script>';
  }
  /**
   * Mentions js
   */
  echo '<script src="' . base_url('modules/chat/assets/js/mentions/underscore.js' . '?v=' . VERSIONING . '') . '"></script>';
  echo '<script src="' . base_url('modules/chat/assets/js/mentions/jquery-elastic.js' . '?v=' . VERSIONING . '') . '"></script>';
  echo '<script src="' . base_url('modules/chat/assets/js/mentions/mentions.js' . '?v=' . VERSIONING . '') . '"></script>';

  if (strpos(CHAT_CURRENT_URI, '/chat/chat_controller/chat_full_view')) {
    /**
     * Audio record js
     */
    if (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on') {
      echo '<script src="' . module_dir_url('chat', 'assets/js/audio/WebAudioRecorder.min.js' . '?v=' . VERSIONING . '') . '"></script>';
      echo '<script src="' . module_dir_url('chat', 'assets/js/audio/WebAudioRecorderOgg.min.js' . '?v=' . VERSIONING . '') . '"></script>';
      echo '<script src="' . module_dir_url('chat', 'assets/js/audio/sound_app.js' . '?v=' . VERSIONING . '') . '"></script>';
    }
  }
}
...
{
  if ( ! strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
    echo '<link href="' . base_url('modules/chat/assets/css/chat_styles.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" >';
  } else {
    chat_check_theme_options();
  }
  // Mutual files for both chat views
  echo '<link href="' . base_url('modules/chat/assets/css/lity.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
  echo '<link href="' . base_url('modules/chat/assets/css/chat_statuses.css') . '" rel="stylesheet" type="text/css"/>';
  echo '<link href="' . base_url('modules/chat/assets/css/mentions.css') . '" rel="stylesheet" type="text/css"/>';
}
...
{
  if ( ! strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
    echo '<script src="' . base_url('modules/chat/assets/js/jscolor.js' . '?v=' . VERSIONING . '') . '"></script>';
  }
  echo '<script src="' . base_url('modules/chat/assets/js/emoparser.js' . '?v=' . VERSIONING . '') . '"></script>';
}
...
function chat_check_theme_options()
{
  if (get_chat_theme_option() == 'light' || get_chat_theme_option() == null) {
    echo '<link href="' . base_url('modules/chat/assets/css/chat_full_view.css?v=' . time()) . '" rel="stylesheet" type="text/css" />';

  } else {
    echo '<link href="' . base_url('modules/chat/assets/css/chat_full_dark_view.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
  }
  echo '<link href="' . base_url('modules/chat/assets/css/lity.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
}
...
{
  $CI = &get_instance();
  $CI->load->model('chat/chat_model', 'chat_model');
  $unreadMessages = $CI->chat_model->getUnread();

  echo '<script src="' . base_url('modules/chat/assets/js/lity.min.js' . '?v=' . VERSIONING . '') . '"></script>';
  echo $CI->load->view('chat/initViewCheck', ['unreadMessages' => $unreadMessages], true);
}
...
{
  $CI = &get_instance();
  echo $CI->load->view('chat/initViewCheckClients');
}
...
function validateChatColorBeforeApply($color, $model_check = '')
{
  ...
  if ($model_check == true && $validColor === '#15213a') {
    return 'unknownColor';
  }
...
{    // Used in chat_full_view as ['prop' => 'class or something else']
  ...
  require('modules/chat/assets/module_includes/Components/' . $name . '.php');
}
