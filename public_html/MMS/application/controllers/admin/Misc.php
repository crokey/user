...
class Misc extends AdminController
{
    ...
    public function tinymce_file_browser()
    {
        ...
        $this->app_css->add('app-css', base_url($this->app_css->core_file('assets/css', 'style.css')) . '?v=' . time(), 'editor-media');
   ...
   public function get_bank_relation_data()
    {
        if ($this->input->post()) {
            $type = $this->input->post('type');
            $data = get_bank_relation_data($type, '', $this->input->post('extra'));
            if ($this->input->post('rel_id')) {
                $rel_id = $this->input->post('rel_id');
            } else {
                $rel_id = '';
            ...
            $relOptions = init_bank_relation_options($data, $type, $rel_id);
            echo json_encode($relOptions);
            die;
        }
    }
  
  public function get_company_relation_data()
    {
        if ($this->input->post()) {
            $type = $this->input->post('type');
            $data = get_company_relation_data($type, '', $this->input->post('extra'));
            if ($this->input->post('rel_id')) {
                $rel_id = $this->input->post('rel_id');
            } else {
                $rel_id = '';
            ...
            $relOptions = init_company_relation_options($data, $type, $rel_id);
            echo json_encode($relOptions);
            die;
        }
    }
