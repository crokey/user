...
function is_rtl($client_area = false)
{
    ...
    if (is_client_logged_in()) {
        ...
        if (isset($GLOBALS['current_user'])) {
            ...
            $CI->db->select('direction')->from(db_prefix() . 'contacts')->where('id', get_staff_user_id());
            $direction = $CI->db->get()->row()->direction;
        }
...
function get_merchant_userid()
{
    ...
    // Query the contacts table to get the userid associated with the staff user ID
    $CI->db->select('userid');
    $CI->db->where('id', $staffUserId);
    $query = $CI->db->get(db_prefix() . 'contacts');

    // If a row is found, return the userid
    if ($query->num_rows() > 0) {
        $row = $query->row();
        return $row->userid;
    }

    // Return false if no userid is found
    return false;
}

function get_is_application_submitted($merchant_id)
{
    $CI = &get_instance();
    ...
    // Query the merchants table to get the is_application_submitted status
    $CI->db->select('is_application_submitted');
    $CI->db->where('userid', $merchant_id);
    $query = $CI->db->get(db_prefix() . 'clients');

    // If a row is found, return the is_application_submitted status
    if ($query->num_rows() > 0) {
        $row = $query->row();
        return $row->is_application_submitted;
    }
...
function get_is_full_access($client_id) {
    $CI = &get_instance();
    $CI->load->database();
    $CI->db->select('is_full_access');
    $CI->db->where('userid', $client_id);
    $query = $CI->db->get(db_prefix() . 'contacts');
    
    if ($query->num_rows() > 0) {
        $row = $query->row();
        log_message('debug', 'is_full_access: ' . $row->is_full_access);  // Add a log to check the output
        return $row->is_full_access;
    }
    return false;  // Consider whether this should return 0 instead of false, depending on how your database is structured
}
...
function get_related_client_id_for_staff() {
    $CI =& get_instance();
    $staff_id = get_staff_user_id();
    
    // Assuming there's a column in tblcontacts that relates to the staff ID (e.g., staff_id)
    // This is an example. You might need to adjust the logic based on your actual schema
    $CI->db->select('userid');
    $CI->db->from('tblcontacts');
    $CI->db->where('id', $staff_id); // Adjust 'staff_id' to your actual column name
    $query = $CI->db->get();

    if ($query->num_rows() > 0) {
        $contact = $query->row();
        return $contact->userid; // This is the client ID (userid in tblclients)
    }
    
    return false; // Return false or appropriate value if no related client found
}
...
function get_bank_user_id()
{
    if (!is_bank_logged_in()) {
        return false;
    }

    return get_instance()->session->userdata('bank_user_id');
}
...
function get_company_user_id()
{
    ...
    return get_instance()->session->userdata('company_user_id');
}
