...
hooks()->add_filter('before_bank_updated', '_format_data_bank', 10, 2);
hooks()->add_filter('before_company_updated', '_format_data_company', 10, 2);
...
function _format_data_bank($data, $id = null)
  {
    foreach (_get_bank_unused_names() as $u) {
        if (isset($data[$u])) {
            unset($data[$u]);
        }
    }
...
function _format_data_company($data, $id = null)
  {
    foreach (_get_company_unused_names() as $u) {
        if (isset($data[$u])) {
            unset($data[$u]);
        }
    }
...
function _get_bank_unused_names()
{
    return [
        'fakeusernameremembered', 'fakepasswordremembered',
        'DataTables_Table_0_length', 'DataTables_Table_1_length',
...
function _get_company_unused_names()
{
    return [
        'fakeusernameremembered', 'fakepasswordremembered',
        'DataTables_Table_0_length', 'DataTables_Table_1_length',
