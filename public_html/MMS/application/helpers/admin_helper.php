...
/*function check_full_access($client_id = '') {
  $client_id = empty($client_id) ? get_staff_user_id() : $client_id;
  
        $CI =& get_instance(); // Get the CI instance

        // Load the Client model if it's not already loaded
        if (!isset($CI->client_model)) {
            $CI->load->model('client_model');
        }

        // Call the function from the model
        $is_full_access = $CI->client_model->get_is_full_access($client_id);
...
function is_admin($staffid = '')
{
    ...
    $result = $CI->db->count_all_results(db_prefix() . 'contacts') > 0 ? true : false;
