        ...
        if (!is_admin()) {
            if (get_option('staff_access_only_assigned_departments') == 1) {
                ...
                if (count($departments_ids) > 0) {
                    array_push($where, 'AND department IN (SELECT departmentid FROM ' . db_prefix() . 'merchant_departments WHERE departmentid IN (' . implode(',', $departments_ids) . ') AND merchid="' . get_staff_user_id() . '")');
                }
            }
        }
