...
<hr />
<?php
    $tabs         = get_company_customer_profile_tabs();
    $current_tabs = get_option('visible_company_customer_profile_tabs');
    if ($current_tabs != 'all') {
        $current_tabs = unserialize($current_tabs);
    }
?>
<div class="form-group">
	<label for="visible_company_customer_profile_tabs" class="control-label"><?php echo _l('visible_tabs'); ?> (<?php echo _l('company_add_edit_profile'); ?>)</label>
	<select name="settings[visible_company_customer_profile_tabs][]" id="visible_company_customer_profile_tabs" multiple class="form-control selectpicker" data-none-selected-text="<?php echo _l('all'); ?>" data-actions-box="true">
