...
<div class="widget relative" id="widget-<?php echo create_widget_id(); ?>" data-name="<?php echo _l('quick_stats'); ?>">
    ...
    <div class="top_stats_wrapper">
        <?php
          // No additional conditions specified
          $_where = "status = 'Prospective'"; // Adjust the where clause to filter by string value 'Prospective'
          
          // Count the total number of banks
          $total_banks = total_rows(db_prefix() . 'banks');
          
          // Count the total number of banks with 'Prospective' status
          $total_banks_in_progress = total_rows(db_prefix() . 'banks', $_where);
          
          // Calculate the percentage of banks that are in progress
          $percent_in_progress_banks = ($total_banks > 0 ? number_format(($total_banks_in_progress * 100) / $total_banks, 2) : 0);
        ?>
        <div class="tw-text-neutral-800 mtop5 tw-flex tw-items-center tw-justify-between">
            <div class="tw-font-medium tw-inline-flex text-neutral-600 tw-items-center tw-truncate">
                ...
                <span class="tw-truncate">
                    <?php echo _l('total_banks_and_progress'); ?> <!-- Adjusted text -->
                </span>
            </div>
            <span class="tw-font-semibold tw-text-neutral-600 tw-shrink-0">
                <?php echo $total_banks_in_progress; ?> / <?php echo $total_banks; ?>
            </span>
        </div>

        <div class="progress tw-mb-0 tw-mt-4 progress-bar-mini">
            <div class="progress-bar progress-bar-success no-percent-text not-dynamic" role="progressbar" aria-valuenow="<?php echo $percent_in_progress_banks; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $percent_in_progress_banks; ?>%" data-percent="<?php echo $percent_in_progress_banks; ?>">
            </div>
        </div>
    </div>
</div>
