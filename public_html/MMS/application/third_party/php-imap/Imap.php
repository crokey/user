...
class Imap
{
   ...
   public function __construct($mailbox, $username, $password, $encryption = false) {
     log_message('debug', 'Imap class constructor called.');
    $enc = '';
...
protected function logImapErrors() {
    $errors = imap_errors();
    $alerts = imap_alerts();

    if ($errors) {
        error_log("IMAP Errors: " . implode("; ", $errors));
        // Here you can also use echo or any logging mechanism you have to display the errors
    }
    if ($alerts) {
        error_log("IMAP Alerts: " . implode("; ", $alerts));
        // Here you can also use echo or any logging mechanism you have to display the alerts
    }
}

// Call this method after operations that might fail
