...
class App_tags
{
  ...
  public function newsave($tags, $rel_id, $rel_type, $staff_id, $is_shared)
    {
        $affectedRows = 0;
        if ($tags == '') {
            if ($this->newrelation_delete($rel_id, $rel_type)) {
                $affectedRows++;
            }
        } else {
            $tags_array = [];
            ...
            // Check if there are removed tags
            $current_tags = get_tags_in($rel_id, $rel_type, $staff_id, $is_shared);
            ...
            foreach ($current_tags as $tag) {
                if (!in_array($tag, $tags_array)) {
                    ...
                    $this->ci->db->where('is_shared', $is_shared);
                    $this->ci->db->where('tag_id', $tag->id);
                    $this->ci->db->delete(db_prefix() . 'taggables');
                    if ($this->ci->db->affected_rows() > 0) {
                        $affectedRows++;
  ...
  public function saveshared($tags, $rel_id, $rel_type, $is_shared)
    {
        $affectedRows = 0;
        if ($tags == '') {
            if ($this->sharedrelation_delete($rel_id, $rel_type, $is_shared)) {
                $affectedRows++;
            }
        } else {
            $tags_array = [];
            ...
            // Check if there are removed tags
            $current_tags = get_tags_in($rel_id, $rel_type, $is_shared);
    ...
    public function relation_delete($rel_id, $rel_type)
    {
     ...
     public function newrelation($rel_id, $rel_type, $staff_id, $is_shared)
    {
        $this->ci->db->where('rel_id', $rel_id);
        $this->ci->db->where('rel_type', $rel_type);
        $this->ci->db->order_by('tag_order', 'ASC');
    ...
    public function newrelation_delete($rel_id, $rel_type, $staff_id, $is_shared)
    {
        $this->ci->db->where('rel_id', $rel_id);
        $this->ci->db->where('rel_type', $rel_type);
        $this->ci->db->where('staff_id', $staff_id);
  ...
  public function sharedrelation($rel_id, $rel_type, $is_shared)
    {
        $this->ci->db->where('rel_id', $rel_id);
        $this->ci->db->where('rel_type', $rel_type);
        $this->ci->db->order_by('tag_order', 'ASC');
    ...
    public function sharedrelation_delete($rel_id, $rel_type, $is_shared)
    {
        $this->ci->db->where('rel_id', $rel_id);
        $this->ci->db->where('rel_type', $rel_type);
        $this->ci->db->where('is_shared', $is_shared);
