...
class Staff_model extends App_Model
{
    public function delete($id, $transfer_data_to)
    {
        ...
        $this->db->where('merchid', $id);
        $this->db->delete(db_prefix() . 'merchant_departments');
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_staff_logged_in() && $id != '' && $id == get_staff_user_id()) {
            $select_str .= ',(SELECT COUNT(*) FROM ' . db_prefix() . 'notifications WHERE touserid=' . get_staff_user_id() . ' and isread=0) as total_unread_notifications, (SELECT COUNT(*) FROM ' . db_prefix() . 'todos WHERE finished=0 AND merchid=' . get_staff_user_id() . ') as total_unfinished_todos';
        }
        ...
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            $staff = $this->db->get(db_prefix() . 'users')->row();
    ...
    public function get_staff_permissions($id)
    {
        ...
        if (!$permissions && !is_array($permissions)) {
            $this->db->where('merch_id', $id);
            $permissions = $this->db->get('merchant_permissions')->result_array();
        ...
        if ($staffid) {
            ...
            if (isset($departments)) {
                foreach ($departments as $department) {
                    $this->db->insert(db_prefix() . 'merchant_departments', [
                        'merchid'      => $staffid,
        ...
        if (sizeof($staff_departments) > 0) {
            ...
            if (isset($departments)) {
                foreach ($departments as $department) {
                    ...
                    $_exists = $this->db->get(db_prefix() . 'merchant_departments')->row();
                    if (!$_exists) {
                        $this->db->insert(db_prefix() . 'merchant_departments', [
                            'merchid'      => $id,
                            'departmentid' => $department,
    ...
    {
        $this->db->where('merch_id', $id);
        $this->db->delete('merchant_permissions');
