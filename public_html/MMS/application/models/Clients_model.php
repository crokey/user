...
class Clients_model extends App_Model
{
  ...
  public function getFormData($clientID) {
    $query = $this->db->select('af.*, afp.*')
                      ->from('tblapplicationform as af')
                      ->join('tblapplicationformpagethree as afp', 'af.client_id = afp.client_id')
                      ->where('af.client_id', $clientID)
                      ->get();
  ...
  function get_is_full_access($client_id) {
    ...
    public function update($data, $id, $client_request = false)
    {
      ...
      // Extract reconciliation data if present
    if (isset($data['column_name'])) {
        $reconciliationData = [
            'client_id' => $id,
            'column_name' => $data['column_name'],
            // Assuming you want to capture a figure as well, ensure it's included in the form and sanitized here
            'column_figure' => isset($data['column_figure']) ? $data['column_figure'] : 0,
            'date_created' => date('Y-m-d H:i:s'),
        ];

        // Remove reconciliation specific data from $data to avoid errors in the following update
        unset($data['column_name'], $data['column_figure']);

        // Insert into reconciliation table
        $this->db->insert('tblreconciliation', $reconciliationData);
     ...
     // Assuming $data['reconciliation'] contains the submitted data structured as:
// ['reconciliation' => [column_id => ['column_figure' => value]]]
if (isset($data['reconciliation']) && is_array($data['reconciliation'])) {
    foreach ($data['reconciliation'] as $columnId => $reconciliationData) {
        $this->db->where('id', $columnId);
        $this->db->update('tblreconciliation', ['column_figure' => $reconciliationData['column_figure']]);
  ...
  /**
     * Deletes a custom column entry from the reconciliation table.
     * 
     * @param int $columnId The ID of the column to delete.
     * @param int $clientId The ID of the client, for additional security check (optional).
     * @return bool True on success, false on failure.
     */
   public function deleteCustomColumn($columnId, $clientId = null) {
    if ($clientId !== null) {
        $this->db->where('client_id', $clientId);
    }
    $this->db->where('id', $columnId);
    $this->db->delete('tblreconciliation');
    if ($this->db->affected_rows() > 0) {
