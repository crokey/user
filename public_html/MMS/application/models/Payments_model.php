...
class Payments_model extends App_Model
{
    ...
    {
        $this->db->select('*,' . db_prefix() . 'transactions.id as id');
        
        $this->db->order_by(db_prefix() . 'transactions.id', 'asc');
    	$this->db->where(db_prefix() . 'transactions.id', $id);
    	$transaction = $this->db->get(db_prefix() . 'transactions')->row();
        if (!$transaction) {
            return false;
        }
