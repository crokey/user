    ...
    public function update($data)
    {
        ...
        foreach ($data['settings'] as $name => $val) {
            ...
            if ($name == 'default_contact_permissions') {
              ...
              } elseif ($name == 'visible_bank_customer_profile_tabs') {
                if ($val == '') {
                    $val = 'all';
                } else {
                    $tabs           = get_bank_customer_profile_tabs();
                    $newVisibleTabs = [];
                    foreach ($tabs as $tabKey => $tab) {
                        $newVisibleTabs[$tabKey] = in_array($tabKey, $val);
                    }
              ...
              } elseif ($name == 'visible_company_customer_profile_tabs') {
                if ($val == '') {
                    $val = 'all';
                } else {
        ...
        if (!in_array('default_contact_permissions', $all_settings_looped)
          ...
          } elseif (!in_array('visible_bank_customer_profile_tabs', $all_settings_looped)
                && in_array('customer_settings', $all_settings_looped)) {
            $this->db->where('name', 'visible_bank_customer_profile_tabs');
            $this->db->update(db_prefix() . 'options', [
                'value' => 'all',
            ]);
            if ($this->db->affected_rows() > 0) {
          ...
          } elseif (!in_array('visible_company_customer_profile_tabs', $all_settings_looped)
                && in_array('customer_settings', $all_settings_looped)) {
            $this->db->where('name', 'visible_company_customer_profile_tabs');
            $this->db->update(db_prefix() . 'options', [
                'value' => 'all',
            ]);
            if ($this->db->affected_rows() > 0) {
