...
class Authentication_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
          ...
          // Subscription expiry check
			$today = date('Y-m-d');
			if ($user->subscription_status == 'one-time' && $user->subscription_end < $today) {
    			log_activity("User login failed - subscription expired [Email: $email]");
    		return [
  ...
  /**
     * Method to check if the session should be locked due to inactivity.
     */
  
   public function check_lock_screen()
    {
        $last_activity = $this->session->userdata('last_activity');
        $lock_after = 15 * 60; // Lock after 15 minutes of inactivity
        
        if (time() - $last_activity > $lock_after && !$this->session->userdata('is_locked')) {
            $this->session->set_userdata('is_locked', true);
            redirect('authentication/lock_screen');
        }
    }
  
  /**
     * Method to handle the unlocking of the session.
     */
    public function unlock_session($password)
    {
        $user_id = $this->session->userdata('user_id'); // Ensure you have a way to get the current user's ID
        $user = $this->get_user_by_id($user_id); // Implement this method to retrieve user details
  ...
  /**
     * Sample method to get a user by ID - you need to implement this based on your user table structure
     */
    private function get_user_by_id($user_id)
    {
        // Assume $table is determined based on whether it's staff or client
    ...
    /**
     * Send set password email for users
     * @param string $email
     */
