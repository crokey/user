        ...
        if (!is_admin()) {
            ...
            if (get_option('staff_access_only_assigned_departments') == 1) {
                ...
                if (count($departments_ids) > 0) {
                    $where = 'AND department IN (SELECT departmentid FROM ' . db_prefix() . 'merchant_departments WHERE departmentid IN (' . implode(',', $departments_ids) . ') AND merchid="' . get_staff_user_id() . '")';
                }
            }
        }
        $_where = '';
    ...
    private function getStaffMembersForTicketNotification($department, $assignedStaff = 0)
    {
        ...
        if ($assignedStaff != 0 && get_option('staff_related_ticket_notification_to_assignee_only') == 1) {
            ...
            foreach ($staff as $member) {
                if (get_option('access_tickets_to_none_staff_members') == 0 && !is_staff_member($member['merchid'])) {
                    continue;
                }
                $staff_departments = $this->departments_model->get_staff_departments($member['merchid'], true);
